...
(straight-use-package '(dm-org-journal-in-current-buffer :type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t))
...
** PlantUML
#+begin_src elisp
  ;;  (use-package plantuml-mode  :init (setq plantuml-executable-path "/opt/homebrew/bin/plantuml" plantuml-default-exec-mode 'executable) :mode (("\\.plantuml\\'" . plantuml-mode)))
