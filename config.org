...
#+begin_src emacs-lisp
          ...
          %?")
          ("u" "Unexpected task" entry (function dm/capture-unexpected-task-into-org-roam-daily))
...
#+begin_src elisp
  (use-package gptel
    :bind (("C-c d g" . gptel-send))

Ayo, boss, check this out. Here's a quick rundown of my setup for Emacs, dig?

- My Emacs init file, ya know? We get it all tangled up using =~/.emacs=.

- Don’t forget: Hittin' ~C-c C-,~ kicks in the Emacs templating system.

- Still needs doing:
  - Gotta change that highlight color, ya feel me? Selecting with a mouse or ~C-SPC~ is like trying to see through fog.

*** Garbage Collection Hack

We got this magic trick from [[https://github.com/emacsmirror/gcmh][Garbage collection magic hack]]. Keeps the system slick by handling garbage collection real smooth-like. During regular use, we set a high GC threshold and when idling, the low threshold kicks in.
...
We try to keep things neat, boss. Load up some key files like so:
